...
export default function DailyFollowupDetailPage() {
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div className="page-header-row">
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-lg)' }}>
            <Link href="/daily-followups" className="btn btn-icon" title="返回列表">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="m15 18-6-6 6-6" /></svg>
            </Link>
            <div>
              <div className="page-eyebrow">DAILY-FOLLOWUP / {String(record['沟通主题'] ?? id.slice(0, 6))}</div>
              <h1 className="page-title">日常跟进详情 · {studentName}</h1>
              <p className="page-subtitle">{ts('subtitleViewOnly')}</p>
            </div>
          </div>
          <div className="page-actions">
            <button className="btn btn-outline btn-sm" onClick={() => router.push('/daily-followups')}>{t('backToList')}</button>
          </div>
        </div>
      </div>
      ...
      {/* ── 关联笔记（得到大脑） ─────────── */}
      <NotePanel entityType="日常跟进" entityId={id} entityName={linkText(record['关联学生'])} />
    </div>
  );
}
