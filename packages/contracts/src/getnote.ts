...
export interface NoteListFilters {
  ...
  status?: string;
  /**
   * 只返回**我自己的笔记**（本人凭证那一路，2026-09-26 新增）。
   *
   * 为什么需要它：非管理员的默认路径在「被关联到某个知识库配置」时，会用**配置自己的凭证**
   * 拉该配置下的**全部**笔记 —— 那是「我的笔记」页面的正确语义（共享知识库要能协作），
   * 但在「把笔记导入到学生 IDP」这个场景里不合适：那是"把我的笔记挂上去"的动作，
   * 列出同事的笔记既容易选错，也不符合峰哥要求的可见性口径。
   *
   * ⚠️ 管理员**不吃这个参数**（管理员本就该看到全部）。
   */
  mine?: boolean;
}
