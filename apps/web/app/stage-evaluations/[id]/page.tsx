      ...
      <div className="page-header">
        <div className="page-header-row">
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-lg)' }}>
            <Link href="/stage-evaluations" className="btn btn-icon" title="返回列表">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="m15 18-6-6 6-6" /></svg>
            </Link>
            <div>
              <div className="page-eyebrow">STAGE-EVALUATION / {String(record['评价日期'] ?? id.slice(0, 6))}</div>
              <h1 className="page-title">阶段评价详情 · {studentName}</h1>
              <p className="page-subtitle">仅查看，不可修改</p>
            </div>
          </div>
          <div className="page-actions">
            <button className="btn btn-outline btn-sm" onClick={() => router.push('/stage-evaluations')}>返回列表</button>
          </div>
        </div>
      </div>
