...
const PERMISSION_LABEL_OVERRIDES: Record<string, string> = {
  ...
  'openplatform:read': '开放平台·查看',
  'openplatform:write': '开放平台·配置',
  'admin:monitor': '系统监控',
  'config:read': '配置查看',
  'config:write': '配置编辑',
  'ai:chat': 'AI 对话',
...
export const DOMAIN_ORDER = [
  ...
  'mail', 'getnote', 'report', 'md', 'openplatform', 'weiling',
  // 菜单级域：排在资源域之后
...
export interface RoleDef {
  ...
  menus?: string[];
  /**
   * 学生档案「数据范围」（2026-09-15）：该角色的人能看到哪些学生的档案。
   * undefined / 空 = 不限制（看全部）；非空 = 维度间 AND、维度内 OR。
