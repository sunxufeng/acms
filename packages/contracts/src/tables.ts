...
export const TABLES = {
  ...
  /**
   * 归属人映射（2026-09-24）：「卫瓴联系人表的归属人」→「ACMS 用户」的对照表。
   *
   * 为什么需要它：卫瓴侧的「归属人」是 `致极学院-曹老师｜Dainel|1510` 这种**复合串**
   * （机构前缀 + 称呼 + 英文名 + 计数后缀，英文名还有错拼），与我们系统的用户姓名对不上。
   * 靠算法猜（姓 + 英文名近似）只能算"兜底"，且**猜错等于让人看到别人的联系人**。
   * 所以把这条对应关系显式配置出来，由管理员维护。
   *
   * 本地表（不同步飞书）：只在系统内建，`SqlStore.ensureTable` 在模块初始化时自动创建。
   */
  ownerMapping: { tableId: 'tblownermap0001', name: '归属人映射表' },
