...
export const DICTIONARIES_RAW: Record<string, string[]> = {
  ...
  /**
   * 入学年份统一采用 `YY春季/秋季` 写法（如 26秋季），与学生名单源表一致。
   * 2026-09-02 由峰哥在字典管理页统一收敛为此 15 项，早前的 `YYYY春/秋` 写法
   * （2021春 … 2027秋）已废弃。
   * ⚠️ 注意：飞书字段的选项由 syncToBase **只追加不删除**，因此 Base 里的
   * `入学年份` 下拉仍残留 14 个 `YYYY春/秋` 旧选项，需要清理得手动全量 PUT 覆盖。
