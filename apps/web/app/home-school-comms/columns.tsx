...
// 学生列点击直接跳转「这一条家校沟通记录」的只读详情页（不再打开编辑表单、不再跳转学生全景）。
export const COLUMNS: CrudColumn[] = [
  ...
  { key: '家长反馈', label: '家长反馈', list: false, form: true, type: 'markdown' },
 ...
 *   沟通时间 / 沟通时长 / 沟通主题，沟通人与责任人默认取当前登录用户。
 *   只填空字段，笔记映射已写入的值不覆盖。
 */
export function parseHomeSchoolFromSummary(
  values: Record<string, unknown>,
  ctx?: { userName?: string },
): Record<string, unknown> {
  return enrichFromNotes(values, COMM_SPEC, { 沟通人: ctx?.userName ?? '', 责任人: ctx?.userName ?? '' });
}
