 ...
 *
 * 🔴 这里是**后端投影白名单**，不是"顺手多给几个字段"：`project()` 只放行名单里的字段，
 *    其余一律降级成占位符。所以前端 `DIMENSION_PANELS` **每加一个维度，这里必须同步加字段** ——
 *    漏加不会报错，只会让那个维面**整维空**（或全是"未填写"），看起来像"这批学生都没填"，
 *    实际是后端根本没把值传出去。两端是同源关系，有单测钉住。
 */
const DIMENSION_FIELDS: readonly string[] = [
  // 2026-09-22：「入学年份」改名「入学年月」（值是 26秋季 这类学年学期）——
  // 维度名必须跟着字段名改，否则分组统计读不到值（整维度空，且不报错）。
  '校区', '当前年级', '入学年级', '入学年月',
  // 2026-09-26：学生结构概览再补两个入学维度。
  //   · 「入学年份」= 2021–2030 纯年份（由「入学年月」派生，生产 82/82 已填满）
  //   · 「Arete入学年」= 第1年–第10年（Arete 第 N 个学年，生产 82/82 已填满）
  //   ⚠️ 这两个与「入学年月」是**三个不同字段**（2026-09-22 腾名字时拆开的），别合并成一项：
  //      「入学年月」到学期（26秋季），「入学年份」到年（2026），「Arete入学年」到学年序（第6年）。
  '入学年份', 'Arete入学年', '是否是新生', '性别',
